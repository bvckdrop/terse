...
def extract_frontmatter(text):
    """Return the raw `---...---` frontmatter block (with delimiters), or "" if none."""
...
def validate_frontmatter(orig, comp, result):
    """Nothing else protects frontmatter from drift, so check it explicitly."""
    f1 = extract_frontmatter(orig)
    f2 = extract_frontmatter(comp)

    if f1 != f2:
        result.add_error("YAML frontmatter changed — must be preserved verbatim")
